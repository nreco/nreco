...
<xsl:stylesheet version='1.0' 
	...
	<xsl:template match="l:bar|l:pie|l:line" mode="googlechart">
		...
		<xsl:variable name="chartParams">
			...
			<xsl:if test="not($title='')">chtt=@@lt;%=WebManager.GetLabel(@@quot;<xsl:value-of select="$title"/>@@quot;,this) %@@gt;|@@amp;</xsl:if>
		...
		<xsl:variable name="chartSrc"><xsl:value-of select="translate($chartParams, '&#xA;&#xD;&#x9;', '')"/></xsl:variable>
		<img class="googlechart" width="{$width}" height="{$height}" alt="{$title}" src='{$chartSrc}'/>
	</xsl:template>
