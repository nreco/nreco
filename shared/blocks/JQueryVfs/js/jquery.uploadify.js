			...
			fileUpload:function(options) {
				if (flashVer >= 9) {
					...
					if (settings.onInit() !== false) {
						...
						$("#" + $(this).attr('id') + "Uploader").after('<table id="' + $(this).attr('id') + 'Queue" class="fileUploadQueue"></table>');
					}
					$(this).bind("rfuSelect", {'action': settings.onSelect}, function(event, queueID, fileObj) {
						if (event.data.action(event, queueID, fileObj) !== false) {
							...
							$('#' + $(this).attr('id') + 'Queue').append('<tr id="' + $(this).attr('id') + queueID + '" class="fileUploadQueueItem">\
									...
									<td><div class="fileUploadProgress">\
										<div id="' + $(this).attr('id') + queueID + 'ProgressBar" class="fileUploadProgressBar" style="width: 1px; height: 16px;"><span class="percentage">&nbsp;</span></div>\
									</div></td>\
								</tr>');
						}
					});
					...
					$(this).bind("rfuCancel", {'action': settings.onCancel}, function(event, queueID, fileObj, data) {
						if (event.data.action(event, queueID, fileObj, data) !== false) {
							$("#" + $(this).attr('id') + queueID).hide(0, function() { $("#" + $(this).attr('id') + queueID).remove()});
						}
					});
					$(this).bind("rfuClearQueue", {'action': settings.onClearQueue}, function() {
						if (event.data.action() !== false) {
							$('#' + $(this).attr('id') + 'Queue').contents().hide(0, function() {$('#' + $(this).attr('id') + 'Queue').empty()});
						}
					});
