	...
	<xsl:template match="l:jscallback" mode="csharp-code">
		...
		if (System.Web.UI.ScriptManager.GetCurrent(Page)!=null @@amp;@@amp; System.Web.UI.ScriptManager.GetCurrent(Page).IsInAsyncPostBack) {
			...
			if (<xsl:value-of select="$context"/> is NReco.Web.ActionContext) {
				( (NReco.Web.ActionContext) ((object)<xsl:value-of select="$context"/>) ).ResponseEndRequested = true;
			} else { 
				Response.End();
			}
		}
			...
			<xsl:call-template name="apply-visibility">
				<xsl:with-param name="content">			
					...
					<div>
						...
						<xsl:if test="@caption">
							<span class="caption"><NReco:Label runat="server"><xsl:value-of select="@caption"/></NReco:Label><xsl:call-template name="renderFormFieldCaptionSuffix"/></span>
						</xsl:if>
						<span class="editor">
							<xsl:apply-templates select="." mode="form-view-editor">
								<xsl:with-param name="mode" select="$mode"/>
