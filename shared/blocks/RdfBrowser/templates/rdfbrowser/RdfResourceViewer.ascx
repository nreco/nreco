			...
			<div id="leftPanel">
				...
				<asp:Repeater runat="server" DataSource='<%# Left %>'>
					<ItemTemplate>
						...
						<div>
							<a href="rdfbrowser.aspx?resource=<%# HttpUtility.UrlEncode( (string)Eval("Property.Uid.Uri") ) %>"><small>(what is <%# Eval("Property.Label") %>)</small></a>
							<br/>
		...
		<td width="<%# CenterWidth %>%" class="centerPanel">
			...
			<div id="centerPanelContent" class="ui-corner-bottom ui-widget-content" style="padding:5px;">
				...
				<asp:Repeater runat="server" DataSource="<%# Center %>">
					<ItemTemplate>
						<tr class="horizontal">
							<th><a href="rdfbrowser.aspx?resource=<%# HttpUtility.UrlEncode( (string)Eval("Property.Uid.Uri") ) %>"><%# Eval("Property.Label") %></a>:</th>
							<td>
								<%# Eval("Value") %>
							</td>
						</tr>
					...
					<ItemTemplate>
						...
						<div>
							...
							<asp:Repeater runat="server" DataSource='<%# Eval("References") %>'>
								<ItemTemplate>
									<div><a href="rdfbrowser.aspx?resource=<%# HttpUtility.UrlEncode( (string)Eval("Uid.Uri") ) %>"><%# Eval("Label") %></a></div>
								</ItemTemplate>
							</asp:Repeater>
						</div>
					</ItemTemplate>
